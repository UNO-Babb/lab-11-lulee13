...
def shopper(env, id):
    arrive = env.now
    items = random.choice([5, 5, 5, 5, 5, 5, 10, 10, 10, 10, 10, 10, 15, 15, 15, 15, 15, 20, 20, 20, 20, 20, 20])
    shoppingTime = items // 1 # shopping takes 1/2 a minute per item.
...
def customerArrival(env):
    ...
    while True:
        ...
        yield env.timeout(0.5) #New shopper every two minutes
